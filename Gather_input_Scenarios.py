...
output_dict = {scenario: {year: {} for year in weather_year_list} for scenario in scenarios}
...
for j in range(len(weather_year_list)):
    ...
    for i in range(len(scenarios)):
        ...
        output_dict[scenario][year] = result_list


# Make a excel workbook file
output_file = os.path.join(folder_name, "scenarios_result.xlsx")

# Reporting (export to .xlsx file)
with pd.ExcelWriter(output_file) as writer:
    for scenario, year_data in output_dict.items():
        ...
        # Write to sheet named after the scenario
        df.to_excel(writer, sheet_name=str(scenario), index=False)
