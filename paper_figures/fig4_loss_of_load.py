...
caption = (
    ...
    "worst single-outage-event duration (h), the tail metric SO-CVaR targets. "
    "Each panel shows the electrical/critical service (left, translucent) and "
    "thermal/HVAC service (right, solid) side by side; bars are grouped by method "
    "(colour) and architecture (hatch). Values are test-split means over 5 test "
    "years then 5 folds; error bars are +/-1 SD across folds. Note the per-climate "
    "y-scales (cold->hot). Electrical/critical loss of load is negligible "
    "everywhere (<=0.7 kWh/yr and <=1.2 h); essentially all unmet load is thermal, "
    "and SO-CVaR lowers thermal unmet energy relative to LP-Avg in every climate."
)
