"""
Data center (DC) scenario: 5-fold cross-validation with 2D sensitivity
...
# VoLL sensitivity: HVAC ($/kWh) and critical electrical ($/kWh) for data center
VOLL_SCENARIOS = {
    "Low": {"hvac_voll": 3.0, "critical_voll": 10.0},
    "Med": {
        "hvac_voll": Input_Parameters.HVAC_lol_cost_DC,
        "critical_voll": Input_Parameters.lossofloadcost_DC,
    },
    "High": {"hvac_voll": 30.0, "critical_voll": 90.0},
}
...
folder_name = "DC_Results"
output_file = os.path.join(folder_name, "DC_Sensitivity_Results.xlsx")
...
def hvac_voll_context(hvac_voll):
    """Temporarily set DC HVAC VoLL (read as HVAC_lol_cost_DC when scenario='DC')."""
    original = Input_Parameters.HVAC_lol_cost_DC
    Input_Parameters.HVAC_lol_cost_DC = hvac_voll
    try:
        yield
    finally:
        Input_Parameters.HVAC_lol_cost_DC = original
