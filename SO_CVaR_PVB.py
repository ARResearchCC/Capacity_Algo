...
def SO_CVaR_PVB_training(
    ...
    """
    Two-stage CVaR-based stochastic optimization with PV and battery sizing only.

    PCM TES capacities are fixed at zero; heat pump size is fixed from Input_Parameters.
    Each training year is a full-year scenario with shared first-stage capacities
    and scenario-specific dispatch. Minimizes:

        first_stage_cost
        + (1 - lambda) * (1/N) * sum_s Z_s
        + lambda * [ eta + (1 / ((1 - alpha) * N)) * sum_s xi_s ]

    where Z_s is the annual VoLL-weighted unmet HVAC (G2H) and critical electrical
    (G2E) load cost in training year s; eta and xi_s linearize CVaR_alpha(Z) via
    Rockafellar-Uryasev. lambda=0 recovers expected-cost SO; lambda=1 uses pure
    CVaR on outage cost.
    ...
    lossofloadcost : float
        Cost of loss of load for critical electrical load.
    capacity_costs : list
        [C_PV, C_PV_OP, C_B, C_B_OP] capacity cost parameters.
    scenario : str
        'DC' or other (FOB, RC) for heat pump and HVAC VoLL settings.
    cvar_alpha : float, optional
        CVaR confidence level in (0, 1). Defaults to Input_Parameters.CVaR_alpha.
    cvar_lambda : float, optional
        Weight on CVaR vs expected outage cost in [0, 1].
        Defaults to Input_Parameters.CVaR_lambda.

    Returns:
    --------
    tuple
        (PV_Size, Battery_Size, PCM_Heating_Size, PCM_Cooling_Size, ObjValue,
         First_stage_cost, Second_stage_cost, HVAC_Cost, Critical_load_cost,
         Expected_outage_cost, CVaR_outage_cost, CVaR_eta)
    """
    ...
    # First-stage variables: PV and battery only (HP fixed; PCM fixed at 0)
    model.PVSize = pyo.Var(within=pyo.NonNegativeReals)
    model.BatterySize = pyo.Var(within=pyo.NonNegativeReals)
    model.PCM_H_Size = pyo.Param(initialize=0.0)
    model.PCM_C_Size = pyo.Param(initialize=0.0)
    ...
    # CVaR linearization (Rockafellar-Uryasev): CVaR_alpha(Z) = eta + (1/((1-alpha)*N)) * sum xi_s
    model.eta = pyo.Var(within=pyo.Reals)
    model.xi = pyo.Var(model.S, within=pyo.NonNegativeReals)

    def scenario_outage_cost_rule(m, s):
        hvac = m.δt * m.HVAC_lol_cost * sum(m.G2H[s, t] for t in m.T)
        critical = m.δt * m.lossofloadcost * sum(m.G2E[s, t] for t in m.T)
        return hvac + critical

    model.Z = pyo.Expression(model.S, rule=scenario_outage_cost_rule)

    def cvar_excess_rule(m, s):
        return m.xi[s] >= m.Z[s] - m.eta
    ...
    # Per-scenario annual VoLL-weighted outage cost Z_s (reporting decomposition)
    ...
    expected_outage_cost = (1 / num_scenarios) * sum(model.Z[s] for s in model.S)
    cvar_outage_cost = model.eta + (1 / ((1 - cvar_alpha) * num_scenarios)) * sum(
        model.xi[s] for s in model.S
    )
    second_stage_cost = (
        (1 - cvar_lambda) * expected_outage_cost + cvar_lambda * cvar_outage_cost
    )

    # Total objective: first-stage + risk-adjusted second stage
    ...
    Expected_outage_cost = round(pyo.value(expected_outage_cost), 3)
    CVaR_outage_cost = round(pyo.value(cvar_outage_cost), 3)
    CVaR_eta = round(pyo.value(model.eta), 3)
    return (
        PV_Size,
        Battery_Size,
        PCM_Heating_Size,
