"""
FOB scenario (PV + battery sizing only): 5-fold cross-validation with 2D sensitivity
(5 locations x 3 VoLL levels).

Compared to FOB.py, the only design difference is that PCM hot/cold TES is not sized
(PCM capacities fixed at 0 kWh). Heat pump capacity is the same fixed default as FOB.py
(Input_Parameters.HPSize for FOB). PV and battery are the sole decision variables.

Methods:
  - LP_Avg:    run LP once per training year, average capacities
  - LP_Worst:  run LP once per training year, keep capacities from highest-cost training year
  - SO_CVaR:   CVaR-based stochastic optimization (SO_CVaR_PVB.py)
"""
...
folder_name = "FOB_PVB_Results"
output_file = os.path.join(folder_name, "FOB_PVB_Sensitivity_Results.xlsx")
...
def run_lp_training(nested_dict, location, training_years, critical_voll):
    ...
    for year in training_years:
        ...
        result = Baseline_CO_PVB.Cap_Baseline_PVB(
...
def run_so_cvar_training(nested_dict, location, training_years, critical_voll):
    ...
    result = SO_CVaR_PVB.SO_CVaR_PVB_training(
        input_df_list,
        critical_voll,
        capacity_costs,
        scenario,
    ...
    config = pd.DataFrame(
        [
            {"Parameter": "Scenario", "Value": scenario},
            {
                "Parameter": "Sizing",
                "Value": (
                    f"PV and battery sized; PCM hot/cold fixed at 0 kWh; "
                    f"HP fixed at {Input_Parameters.HPSize} kW (Input_Parameters.HPSize)"
                ),
            },
